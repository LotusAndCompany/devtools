...
UIデザインファイル（`.pen`）は `designs/` ディレクトリに格納されています。これらのファイルは `.gitattributes` でバイナリとして扱われるため、Gitによるテキストベースの自動マージはできません。

#### マージコンフリクトの解消

`.pen` ファイルでマージコンフリクトが発生した場合、Gitは自動マージを試行しません。どちらのバージョンを採用するか手動で選択する必要があります：

```bash
# 自分のバージョン（現在のブランチ）を採用
git checkout --ours designs/devtools-design.pen
git add designs/devtools-design.pen

# 相手のバージョン（マージ元ブランチ）を採用
git checkout --theirs designs/devtools-design.pen
git add designs/devtools-design.pen
```

両方のブランチの変更を統合する必要がある場合は、それぞれのバージョンをPencilエディタで開いて手動で統合してください：

1. 一方のブランチのファイルを一時的な場所に保存
2. `--ours` または `--theirs` で一方のバージョンを選択してコンフリクトを解消
3. 両方のファイルをPencilエディタで開き、デザインの変更を手動でマージ
4. 最終結果をコミット

#### ベストプラクティス

- 共有の `.pen` ファイルを編集する前に、同時編集を避けるためチームと**コミュニケーション**を取る
- 大きなデザイン変更は可能な限り別々のPRに分割し、**変更を小さく保つ**
- レビュアーがデザインの変更内容を理解できるよう、**説明的なコミットメッセージ**を書く
