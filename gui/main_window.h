...
public:
     ...
     *          QMenuBar::triggered(QAction *action) → ::onActionTriggered(QAction *action)@n
     *          Sidemenu::itemSelected(Sidemenu::ID id) → ContentsArea::onSidemenuItemChanged(Sidemenu::ID id)@n
     *          QPushButton::clicked() → ::onSidemenuVisibilityButtonClicked()@n
     *          QPushButton::toggled(bool checked) → ::onWindowAlwaysOnTopButtonToggled(bool checked)
     */
...
private:
     ...
     *          ウィンドウを表示する処理は GuiApplication::event(QEvent *event) で実装
     */
