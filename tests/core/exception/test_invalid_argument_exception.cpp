...
private slots:
    ...
    static void test_argumentReasonConstructor();
    void test_clone();
    void test_raise();
};
...
{
    InvalidArgumentException<int> const e;
...
void TestInvalidArgumentException::test_messageConstructor()
{
    ...
    InvalidArgumentException<int> const e(msg);
...
{
    {
        ...
        InvalidArgumentException const e_int(0xf, reason_int);
        // コンストラクタでメッセージを設定した場合、想定通りの文字列が設定されること
...

    {
        ...
        InvalidArgumentException const e_double(2.718281828459045235360287471352, reason_double);
        // コンストラクタでメッセージを設定した場合、想定通りの文字列が設定されること
...

    {
        ...
        InvalidArgumentException const e_str(QString("qwerty"), reason_str);
        // コンストラクタでメッセージを設定した場合、想定通りの文字列が設定されること
...
void TestInvalidArgumentException::test_clone()
{
    ...
    QCOMPARE_EQ(dynamic_cast<InvalidArgumentException<int> *>(copied.get())->message, msg);
...
{
    const QString msg = rd.nextQString(length);
    InvalidArgumentException<int> const src(msg);
