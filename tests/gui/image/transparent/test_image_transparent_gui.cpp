...
public:
    explicit ImageTransparentMock(QObject *parent = nullptr) : ImageTransparentInterface(parent) {}
...
void TestImageTransparentGUI::test_onTransparencyValueChanged()
{
    ...
    // [0, 256)の不透明度が設定されること
    QCOMPARE_EQ(gui.imageTransparent->opacity,
                static_cast<uchar>(255 * (1.0 - randomTransparency)));

    // 例外が発生すること
    QVERIFY_THROWS_EXCEPTION(InvalidArgumentException<double>,
                             gui.onTransparencyValueChanged(-0.01));
    QVERIFY_THROWS_EXCEPTION(InvalidArgumentException<double>,
                             gui.onTransparencyValueChanged(1.01));
}
