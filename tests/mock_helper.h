...
template<typename T>
T _DefaultValue<T &>::value;

/**
 * @brief 渡された引数を記憶していく関数オブジェクト。引数が複数のバージョン。
 * @tparam arg_type 引数の型
 */
...
struct _ArgumentsHistory
{
     ...
     * @return 引数の履歴
     */
    inline const element_type &at(int index) const
    {
        if (index < 0)
