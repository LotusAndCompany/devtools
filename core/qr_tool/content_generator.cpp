...
QString ContentGenerator::generate(const QString &type, const QVariantMap &params) {
    if (type == "calendar") return generateCalendarEvent(params);
    if (type == "contact") return generateContactInfo(params);
    if (type == "email") return generateEmail(params);
    if (type == "geo") return generateGeo(params);
    if (type == "phone") return generatePhone(params);
    if (type == "sms") return generateSms(params);
    if (type == "text") return generateText(params);
    if (type == "url") return generateUrl(params);
    if (type == "wifi") return generateWifi(params);
...
QString ContentGenerator::generateCalendarEvent(const QVariantMap &params) {
    ...
    QString summary = params["summary"].toString();
    QString start   = params["start"].toString();
    QString end     = params["end"].toString();
...
QString ContentGenerator::generateContactInfo(const QVariantMap &params) {
    ...
    if (!phone.isEmpty()) vcard += QString("\nTEL:%1").arg(phone);
    if (!email.isEmpty()) vcard += QString("\nEMAIL:%1").arg(email);
    vcard += "\nEND:VCARD";
...
QString ContentGenerator::generateWifi(const QVariantMap &params) {
    ...
    QString ssid     = params["ssid"].toString();
    QString password = params.value("password").toString();
    QString type     = params.value("type", "WPA").toString(); // WPA/WEP/nopass

    return QString("WIFI:T:%1;S:%2;P:%3;;")
        .arg(type, ssid, password);
}
