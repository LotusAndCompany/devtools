 ...
 * @file
 * @brief 整数→enumのキャスト
 * @details 整数型を(比較的)安全に最小値=MIN,
 * 最大値=MAXかつ値が連続であるenum型に変換するテンプレートを提供する@n MIN,
 * MAXへの変換は許可しない@n 要件を満たさないenum型に対しては特殊化が必要
 */
...
struct enum_cast
{
     ...
     * @brief コンストラクタ
     * @param value 変換する整数値
     * @exception InvalidArgumentException &lt;int_type&gt; value <= enum_type::MIN または
     * enum_type::MAX <= value の場合
     */
    explicit enum_cast(int_type value) noexcept(false)
    {
        if (MIN_VALUE < value && value < MAX_VALUE) {
            ...
            // NOTE:
            // valueがコンパイル時定数の場合にはコンパイルエラーになるのが望ましいが、恐らく無理
            throw InvalidArgumentException<int_type>(
                value, QString("value must be in range (%1, %2)").arg(MIN_VALUE).arg(MAX_VALUE));
        }
    }
