...
ImageDivision::ImageDivision(QObject *parent) : ImageDivisionInterface(parent) {}
...
bool ImageDivision::saveImpl(bool (*saveFunc)(const QString &, const QImage &, const char *, int),
                             const QString &path, const char *format, int quality) const
{
...
QSizeF ImageDivision::computedCellSize() const
{
    ...
    return {computedCellSizeInternal(size.width(), horizontal),
...
unsigned int ImageDivision::numberOfDivisionInternal(unsigned int sourceSize,
