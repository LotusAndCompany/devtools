...
ImageTransparent::ImageTransparent(QObject *parent) : ImageTransparentInterface(parent) {}
...
void ImageTransparent::addTransparentPixel(const QPoint &start)
{
    ...
    while (static_cast<int>(!lookup.empty()) != 0) {
...
void ImageTransparent::validateColorSpec(QColor::Spec colorSpec) noexcept(false)
{
    ...
    for (int const i : avaliableColorSpecList) {
